...
public class Xoroshiro128Plus
{
    ...
    UInt64 splitmix64(ref UInt64 x)
    {
    ...
    {
        return (NextLong() & ((1L << 24) - 1)) * (1.0f / (1L << 24));
    }

    public int WeightedInt(int[] weights)
    {
        int total = weights.Sum();
        int roll = RangeInt(0, total);
    ...
    {
        return minInclusive + (int)RangeUInt((uint)(maxExclusive - minInclusive));
    }

    public uint RangeUInt(uint minInclusive, uint maxExclusive)
    {
        return minInclusive + RangeUInt(maxExclusive - minInclusive);
    }

    public uint RangeUInt(uint maxExclusive)
    {
        int shift = 32 - GetBitCount(maxExclusive);
        uint number;
        do number = NextUInt() >> shift;
        while (number >= maxExclusive);
        return number;
    }

    public static int GetBitCount(uint number)
    {
        int i = 0;
        for (; number != 0u; ++i)
            number >>= 1;
