...
Coal::operator Vec2() const
{
	...
	case Coal::Type::Object:
	{
		...
		if (it != members.end())
			vector.x = it->second.real;
	}
	{
		auto it = members.find("y");
		if (it != members.end())
			vector.y = it->second.real;
	}
	break;
	case Coal::Type::Array:
		...
		case 0:
			break;
		case 1:
			vector = Vec2(elements[0].real);
			break;
		default:
			vector.x = elements[0].real;
			vector.y = elements[1].real;
			break;
		}
		break;
	default:
...
Coal::operator Vec3() const
{
	...
	{
		auto it = members.find("z");
		if (it != members.end())
			vector.z = it->second.real;
	}
	break;
	case Coal::Type::Array:
		switch (elements.size())
